...
path_to_save <- "~/rds/rds-durbin-group-8b3VcZwY7rY/projects/cichlid/CT-data/ak2272/Results/Figures/ANGSD/geno-pheno-scatter"
...
plot_names <- list() 
counter <- 1
for (var in 18:length(Variables)){
  ...
  for (peak in 1:nrow(tb)){
    ...
    file.name.species <- paste("Var",var.file,"_",peak,"_","species.png",sep = "")
    plot_names[[counter]] <- file.name.species
    # Plotting the scatter plot for species 
    plot_list[[counter]] <- ggplot(D,aes(x=X,y=Y,color=Species)) + geom_point() + labs(x="MAP Genotype (RR/RA/AA)",y=paste("Var",var.file,"trait",sep = " "),title = plot.title)
    counter <- counter +1 
    # File name for population 
    file.name.population <- paste("Var",var.file,"_",peak,"_","population.png",sep = "")
    plot_names[[counter]] <- file.name.population
    # Plotting the scatter plot for population 
    plot_list[[counter]] <- ggplot(D,aes(x=X,y=Y,color=Population)) + geom_point() + labs(x="MAP Genotype (RR/RA/AA)",y=paste("Var",var.file,"trait",sep = " "),title = plot.title)
    counter <- counter +1 
     }
  }
  # Saving the plots 
